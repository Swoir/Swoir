...
public class Swoir {
    ...
    public func createCircuit(manifest url: URL, size: UInt32? = nil, lowMemoryMode: Bool = false, storageCap: UInt64? = nil) throws -> Circuit {
        let circuit = try Circuit(backend: self.backend, manifest: url, size: size, lowMemoryMode: lowMemoryMode, storageCap: storageCap)
        return circuit
    }

    public func createCircuit(manifest data: Data, size: UInt32? = nil, lowMemoryMode: Bool = false, storageCap: UInt64? = nil) throws -> Circuit {
        let circuit = try Circuit(backend: self.backend, manifest: data, size: size, lowMemoryMode: lowMemoryMode, storageCap: storageCap)
        return circuit
    }
}
